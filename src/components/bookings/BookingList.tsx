import { BookingType, UserRoles, type BookingStatus } from "@prisma/client";
    ...
    <div className="items group flex items-center gap-10">
      ...
      <div>
        ...
        {session?.user.role === UserRoles.VET && (
          <Text>
            Cliente: <span className="capitalize">{booking.user.name}</span>
          </Text>
        )}
      </div>
    </div>
...
export default function BookingList({
  ...
  status: BookingStatus; // TODO : remove (move to context)
}) {
  return (
